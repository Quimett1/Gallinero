const CloudExcel = (() => {
  ...
  const app = window.msal ? new msal.PublicClientApplication({ auth: { clientId, authority: 'https://login.microsoftonline.com/common', redirectUri: window.location.origin + window.location.pathname }, cache: { cacheLocation: 'localStorage' } }) : null;
  ...
  async function token(interactive = false) {
    ...
    await app.handleRedirectPromise(); account ||= app.getActiveAccount() || app.getAllAccounts()[0];
    if (!account) { if (!interactive) throw new Error('Inicia sesión con OneDrive.'); const response = await app.loginPopup({ scopes }); account = response.account; app.setActiveAccount(account); return response.accessToken; }
    try { return (await app.acquireTokenSilent({ account, scopes })).accessToken; }
    catch { if (!interactive) throw new Error('Inicia sesión con OneDrive.'); return (await app.acquireTokenPopup({ account, scopes })).accessToken; }
  }
  ...
  return { signIn: () => token(true), load, appendEggs, appendSale, appendExpense, isAvailable: () => Boolean(app), account: () => account };
})();
